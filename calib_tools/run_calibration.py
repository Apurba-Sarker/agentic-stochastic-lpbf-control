...
def run_calibration(
    ...
    plot_distribution_kde(
        exp_w, exp_d, sim_w, sim_d, _empty, _empty, _empty, _empty,
        save_path=dist_png,
        sim_label="MCMC calibrated",
        info_text=f"n_steps={n_steps}  burn_in={burn_in}  accept={mcmc_res['accept_rate']:.3f}",
    )
    plot_scatter_wd(
        exp_w, exp_d, sim_w, sim_d, _empty, _empty, _empty, _empty,
        save_path=scatter_png,
        sim_label="MCMC calibrated",
        info_text=title,
    )
    ...
    return {
        ...
        "_mcmc_raw": mcmc_res,   # in-memory only; gives agent access to posterior_chain for ESS
    }
