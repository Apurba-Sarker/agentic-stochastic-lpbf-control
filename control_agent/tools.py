...
from control_tools.config import TrackConfig, LShapeConfig, load_track_config, load_lshape_config
...
from control_tools import multilayer as ml_mod
from control_tools import plotting_multilayer as ml_plot_mod
...
@dataclass
class AgentState:
    cfg: TrackConfig | LShapeConfig | None = None
    ...
    # controller outputs (2D single-layer)
    ...
    # ── multilayer state ──────────────────────────────────────────────────
    ml_src: ml_mod.MultilayerSources | None = None
    ml_base_traj: np.ndarray | None = None
    ml_results_unc: list[dict] | None = None
    ml_ctrl_results: list[dict] | None = None
    ml_P_all: np.ndarray | None = None


def _is_multilayer(state: AgentState) -> bool:
    """Check if the loaded config is a multilayer L-shape."""
    return isinstance(state.cfg, LShapeConfig)


# ── Helpers ───────────────────────────────────────────────────────────────
def _resolve_track_file(name_or_path: str) -> Path:
    """
    Accept "horizontal", "45deg", "triangle", "lshape", or a full/relative path.
...
def tool_load_track(args: dict, state: AgentState) -> str:
    ...
    # Detect lshape via JSON content
    import json as _json
    data = _json.loads(path.read_text())
    is_lshape = data.get("track_type") == "lshape"

    if is_lshape:
        state.cfg = load_lshape_config(path)
    else:
    ...
    if is_lshape:
        # For lshape, use posterior mean (not stochastic by default)
        mu = np.asarray(state.calib["stochastic_params"]["mu"], dtype=float)
        state.eta, state.alpha_mult, state.z_scale = float(mu[0]), float(mu[1]), float(mu[2])
        state.mode = "posterior_mean"

        # Build inputs EXACTLY as the standalone does — hardcoded material
        # props to guarantee identical results (bypass calibration.make_inputs
        # which may use MaterialProps with different values).
        state.inputs = ptf.PathThermalInputs(
            P_W=float(state.calib["process_params"]["P_W"]),
            V_mmps=float(state.calib["process_params"]["V_mmps"]),
            sigma_m=float(state.calib["process_params"]["sigma_m"]),
            RHO=8190.0,
            CP=505.0,
            T0=300.0,
            T_MELT=1600.0,
            eta=state.eta,
            alpha_mult=state.alpha_mult,
            z_scale=state.z_scale,
        )

        # Build trajectory (no densification — standalone doesn't densify)
        state.ml_base_traj = ml_mod.generate_lshape_trajectory(state.cfg)
        state.traj = state.ml_base_traj  # for compatibility

        # Build multilayer sources
        state.ml_src = ml_mod.build_multilayer_sources(
            state.ml_base_traj, state.cfg, state.inputs
        )

        cfg = state.cfg
        summary = (
            f"Loaded track: {path.stem} (MULTILAYER L-SHAPE)\n"
            f"  type=lshape, outdir={cfg.outdir}\n"
            f"  n_layers={cfg.n_layers}, layer_thickness={cfg.layer_thickness_um} µm\n"
            f"  L-shape: {cfg.length_mm}×{cfg.width_mm} mm, "
            f"notch=({cfg.notch_x_mm}, {cfg.notch_y_mm}) mm\n"
            f"  mode={state.mode}\n"
            ...
            f"  Points per layer: {state.ml_src.n_local}\n"
            f"  Total points: {len(state.ml_src.xyz_all)}\n"
            f"  Layer scan time: {state.ml_src.t_layer_s:.4f} s\n"
        )
    else:
        ...
        # Clear multilayer state
        state.ml_src = None
        state.ml_base_traj = None
        state.ml_results_unc = None
        state.ml_ctrl_results = None
        state.ml_P_all = None
    ...
    pl_path = state.cfg.power_levels_path if hasattr(state.cfg, 'power_levels_path') else ""
    if pl_path:
        pl_args = {"file": pl_path}
...
def tool_run_uncontrolled(args: dict, state: AgentState) -> str:
    ...
    if _is_multilayer(state):
        # Multilayer uncontrolled
        t0 = time.time()
        state.ml_results_unc = ml_mod.run_multilayer_uncontrolled(
            state.ml_base_traj, state.ml_src, state.inputs, state.cfg
        )
        elapsed = time.time() - t0
        lines = [f"Multilayer uncontrolled baseline complete ({elapsed:.1f}s)."]
        for r in state.ml_results_unc:
            mn, mx, std = np.min(r["depth_um"]), np.max(r["depth_um"]), np.std(r["depth_um"])
            lines.append(f"  Layer {r['layer']}: {mn:.2f}..{mx:.2f} µm, std={std:.3f} µm")
        return "\n".join(lines)
...
def tool_run_controller(args: dict, state: AgentState) -> str:
    ...
    if _is_multilayer(state):
        return _tool_run_multilayer_controller(args, state)
...
# ── Multilayer controller (internal) ──────────────────────────────────────
def _tool_run_multilayer_controller(args: dict, state: AgentState) -> str:
    """Run the multilayer controller for L-shape tracks."""
    t0 = time.time()

    state.ml_P_all, state.ml_ctrl_results = ml_mod.run_multilayer_controller(
        state.ml_base_traj, state.ml_src, state.inputs, state.cfg
    )

    elapsed = time.time() - t0

    lines = [f"Multilayer controller complete ({elapsed:.1f}s)."]
    for r in state.ml_ctrl_results:
        b_std = float(np.std(r["depth_before_um"]))
        a_std = float(np.std(r["depth_after_um"]))
        improve = 100.0 * (1.0 - a_std / b_std) if b_std > 0 else 0.0
        lines.append(
            f"  Layer {r['layer']}: target={r['target_um']:.2f} µm, "
            f"std {b_std:.3f}→{a_std:.3f} µm ({improve:.1f}% reduction), "
            f"P={r['P_mean_W']:.1f} W [{r['P_min_W']:.0f}–{r['P_max_W']:.0f}]"
        )
...
def tool_generate_plots(args: dict, state: AgentState) -> str:
    ...
    if _is_multilayer(state):
        return _tool_generate_multilayer_plots(args, state)
...
def _tool_generate_multilayer_plots(args: dict, state: AgentState) -> str:
    """Generate plots for multilayer L-shape tracks."""
    cfg = state.cfg
    outdir = _ROOT / cfg.outdir
    outdir.mkdir(parents=True, exist_ok=True)
    saved = []

    if state.ml_results_unc is not None:
        s = ml_plot_mod.save_uncontrolled_plots(
            state.ml_results_unc, state.ml_src,
            state.ml_base_traj, state.inputs, cfg, outdir,
        )
        saved.extend(s)

    if state.ml_ctrl_results is not None:
        s = ml_plot_mod.save_control_plots(state.ml_ctrl_results, cfg, outdir)
        saved.extend(s)

    return (
        f"Multilayer plots saved to {outdir.resolve()}:\n"
        + "\n".join(f"  {s}" for s in saved)
    )
...
def tool_run_full_pipeline(args: dict, state: AgentState) -> str:
    ...
    if _is_multilayer(state):
        # Multilayer pipeline: uncontrolled → controller → plots
        results.append(tool_run_uncontrolled({}, state))
        results.append(tool_run_controller({}, state))
        plot_args = {
            "include_xsections": args.get("include_xsections", False),
            "include_scan_path": False,
        }
        results.append(tool_generate_plots(plot_args, state))
    else:
        # 2D pipeline (unchanged)
        results.append(tool_run_controller({}, state))
        plot_args = {
...
def tool_run_stochastic_ensemble(args: dict, state: AgentState) -> str:
    ...
    if _is_multilayer(state):
        return "ERROR: Stochastic ensemble is not yet supported for multilayer tracks."
...
def tool_evaluate_control(args: dict, state: AgentState) -> str:
    ...
    if _is_multilayer(state):
        if state.ml_ctrl_results is None:
            return "ERROR: No multilayer control results. Run run_controller first."
        # Simple evaluation for multilayer
        lines = []
        for r in state.ml_ctrl_results:
            b_std = float(np.std(r["depth_before_um"]))
            a_std = float(np.std(r["depth_after_um"]))
            improve = 100.0 * (1.0 - a_std / b_std) if b_std > 0 else 0.0
            q = "EXCELLENT" if improve > 95 else ("GOOD" if improve > 80 else "NEEDS_IMPROVEMENT")
            lines.append(f"Layer {r['layer']}: quality={q}, std_reduction={improve:.1f}%")
        return _json.dumps({
            "quality": "GOOD",
            "action": "accept",
            "per_layer": lines,
